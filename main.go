...
var (
	ctx   context.Context
	client *genai.Client // Make client globally accessible
	model *genai.GenerativeModel
	// Store the current model name for display and error messages
	currentModelName string
	env              string // Make env globally accessible
	startupChannelID = "361961924059070466" // Specific channel ID for startup message
	startupGuildID   = "361961924059070464"   // Specific guild ID for startup message
)
		...
		"setmodel": func(s *discordgo.Session, i *discordgo.InteractionCreate) {
			...
			if err != nil {
				...
				s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
					Type: discordgo.InteractionResponseChannelMessageWithSource,
					Data: &discordgo.InteractionResponseData{
						Content: "Error fetching available models. Please try again later.",
					},
				})
				return
			}
			...
			if len(models) == 0 {
				s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
					Type: discordgo.InteractionResponseChannelMessageWithSource,
					Data: &discordgo.InteractionResponseData{
						Content: "No Gemini models available for content generation.",
					},
				})
				return
			}
...
func messageCreate(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	go func() { // Run in a goroutine to avoid blocking Discord session
		// Send only the current user message to Gemini
		...
		if len(resp.Candidates) > 0 && resp.Candidates[0].Content != nil && len(resp.Candidates[0].Content.Parts) > 0 {
			...
			// Send the Gemini response as a reply to the user's message
			_, err = s.ChannelMessageSendReply(m.ChannelID, geminiResponse, m.Reference())
			if err != nil {
				log.Printf("Error sending reply to message %s in channel %s: %v", m.ID, m.ChannelID, err)
				sendMessage(s, m.ChannelID, "Sorry, I couldn't send the response as a reply.")
			}
		} else {
			sendMessage(s, m.ChannelID, "Sorry, I couldn't get a response from Gemini.")
		}
	}()
}
